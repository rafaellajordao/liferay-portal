...
describe('VerticalTimeline', () => {
	...
	describe('session row', () => {
		...
		it('renders the individual\'s name as a link when a url is provided', () => {
			...
			expect(screen.getByText('Ada Lovelace').closest('a')).toHaveAttribute(
				'href',
				SESSION_ITEM.individualUrl
			);
		});

		it('renders the individual\'s name as plain text when there is no url', () => {
			renderTimeline({
				items: [{...SESSION_ITEM, individualUrl: undefined}]
			});

			expect(screen.getByText('Ada Lovelace').closest('a')).toBeNull();
		});

		it('shows the raw id on its own line for an anonymous session', () => {
			renderTimeline({
				items: [
					{
						...SESSION_ITEM,
		...
		it('does not show the individual block on a session without an individualName', () => {
			const {container} = renderTimeline({
				items: [{...SESSION_ITEM, individualName: undefined}]
			});

			expect(
				container.querySelector('.individual-name')
		...
		it('does not show the raw attributes JSON when the session row itself is expanded', () => {
			const {container} = renderTimeline({
				items: [
					{
						...SESSION_ITEM,
		...
		it('reveals its pages when expanded', () => {
			const {container} = renderTimeline({
				items: [
					{
						...SESSION_ITEM,
	...
	describe('page group row', () => {
		...
		it('reveals the session\'s attributes and its own events when expanded', () => {
			const {container} = renderTimeline({
				items: [
					{
						...PAGE_ITEM,
				...
				sessionAttributes: {header: 'Session Attributes', userAgent: 'Mozilla/5.0'}
			});
